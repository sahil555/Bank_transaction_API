...
app.config["MONGO_URI"] = "mongodb://flaskapp:27017"

mongoclient = pym(app)


def UserExist(username):
    if mongoclient.db.users.find({"Username":username}).count() == 0:
        return False
    else:
        return True
...
class Register(Resource):
    def post(self):
        ...
        mongoclient.db.users.insert_one({
            "Username": username,
            "Password": hashed_pw,
            "Own":0,
            "Debt":0
...
def verifyPw(username, password):
    ...
    hashed_pw = mongoclient.db.users.find({
        "Username":username
    })[0]["Password"]
...
def cashWithUser(username):
    cash = mongoclient.db.users.find({
        "Username":username
    })[0]["Own"]
    return cash

def debtWithUser(username):
    debt = mongoclient.db.users.find({
        "Username":username
    })[0]["Debt"]
    return debt
...
def updateAccount(username, balance):
    mongoclient.db.users.update_one({
        "Username": username
    },{
        "$set":{
            "Own": balance
...
class Balance(Resource):
    def post(self):
        ...
        retJson = mongoclient.db.users.find({
            "Username": username
        },{
